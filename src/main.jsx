...
const projectMedia = {
  ...
  '02': [{ source: '/assets/campus-ai-work-web.mp4', kind: 'video' }, { source: '/assets/campus-shen-sui-web.mp4', kind: 'video' }],
    ...
    {galleryOpen && <div className="other-work-lightbox" role="dialog" aria-modal="true" onWheel={(event) => { event.preventDefault(); if (Math.abs(event.deltaY) > 10) changeOtherWork(event.deltaY > 0 ? 1 : -1) }}>
      ...
      <div className="project-player-window">
        ...
        <div className="project-player-media" onContextMenu={(event) => event.preventDefault()}>
          {current.kind === 'image' ? <img src={current.source} alt="Project artwork" decoding="async" draggable={false} onContextMenu={(event) => event.preventDefault()} /> : <video ref={playerRef} key={current.source} src={current.source} autoPlay muted playsInline preload="metadata" controls controlsList="nodownload noplaybackrate noremoteplayback" disablePictureInPicture draggable={false} onContextMenu={(event) => event.preventDefault()} />}
        </div>
