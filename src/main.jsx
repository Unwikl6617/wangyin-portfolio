...
const projects = [
  { id: '01', type: 'VISUAL SYSTEM / MOTION', title: 'AI 动画视觉叙事', body: '从概念设定到动态镜头，以 AI 辅助建立可延展的视觉叙事语言。' },
  { id: '02', type: 'CAMPUS AI / 精选作品', title: '校园AI作品', body: '围绕校园创作实践完成的优秀 AI 影像作品，以镜头语言与视觉想象呈现叙事表达。' },
  { id: '03', type: 'AI IMAGE / COLLECTION', title: 'AI 图集作品展示', body: '二十张 AI 图像作品，涵盖人物、场景与风格化视觉实验。' },
]
const capabilities = [
  ['01', 'AI 内容生产', '从需求拆解、分镜到素材生成与后期交付，建立稳定的 AI 内容生产链路。'],
...
function App() {
  ...
  return <main>
    ...
    <section className="hero" id="top">
      ...
      <nav className="nav shell"><a className="wordmark" href="#top">UNWIKL'S WORD</a><div className="nav-links"><a className={activeSection === 'about' ? 'is-active' : ''} href="#about">关于我</a><a className={activeSection === 'work' ? 'is-active' : ''} href="#work">作品精选</a><a className={activeSection === 'ability' ? 'is-active' : ''} href="#ability">能力体系</a></div><a className="contact-pill" href="mailto:13662852993@163.com">联系我 <Arrow /></a></nav>
      <div className={'hero-signature ' + (heroScan ? 'is-scanning' : '')}><p onPointerLeave={triggerHero}>{['王','胤'].map((letter, index) => <span data-letter={letter} key={letter + index}>{letter}</span>)}</p><small>You can also call me Unwikl</small></div>
    ...
    <section className="about section shell" id="about"><div className="section-label"><span>01</span> PROFILE</div><div className="about-grid">
      <ElectricBorder className="portrait-panel" color="#b68aff"><div className="portrait-wrap"><img src="/assets/wangyin-photo.png" alt="Wangyin portrait" draggable={false} onContextMenu={(event) => event.preventDefault()} /><div className="portrait-mark">Unwikl</div></div><p className="portrait-intent">求职意向｜AI设计师｜AI制片｜AI导演向</p></ElectricBorder>
      <div className="about-copy"><div className="about-intro"><p className="eyebrow">HELLO, I'M</p><h2 className={'about-name ' + (nameScan ? 'is-scanning' : '')} onPointerLeave={triggerName}><span className="name-letter" data-letter="W">WANG</span><span className="name-letter" data-letter="Y">YIN</span><span className="about-role" data-role="AI Designer">AI Designer</span></h2><p className="intro">专注于 AI 内容生产与视觉创作，熟悉动画制作流程。通过 AI 工具与设计判断的结合，推动内容生产、视觉创作及团队协同效率提升。</p></div><div className="education"><span>教育背景</span><div><b>2022.09—2026.07</b><b>华南农业大学</b><b>动画专业</b><b>本科</b></div></div><div className="contact-lines"><button className="contact-copy" type="button" onClick={() => notify('WeChat signal saved. Search Unwikl.') }><span>WECHAT</span>Unwikl</button><button className="contact-copy" type="button" onClick={copyEmail}><span>MAIL</span>13662852993@163.com</button><p><span>BASED IN</span>广州 / 中国</p></div></div>
    </div></section>

    <section className="works section" id="work"><div className="shell"><div className="works-heading"><div className="section-label"><span>02</span> 精选作品</div><h2>精选作品</h2><p>所有作品均只作为求职展示</p></div><div className="project-list">{projects.map((project) => <ElectricBorder className="project-case-panel" color="#b68aff" key={project.id}><article className="project-case"><div className="project-video-slot"><div className="project-video-grid" /><span className="video-index">{project.id}</span><div className="video-placeholder"><i>PLAY</i><p>PROJECT MEDIA<br />SCROLL TO VIEW</p></div></div><div className="project-case-copy"><p className="project-type">{project.type}</p><h3>{project.title}</h3><p className="project-summary">{project.body}</p><button type="button" className="project-detail" onClick={() => openProject(project.id)}>查看项目 <Arrow /></button></div></article></ElectricBorder>)}</div></div></section>

    <section className="ability section shell" id="ability"><div className="ability-header"><div className="section-label"><span>03</span> WHAT I DO</div><h2>让灵感落地<br />让流程更快</h2></div><div className="capability-list">{capabilities.map(([number, title, body]) => <ElectricBorder className="capability-panel" color="#b68aff" key={number}><article className="capability"><span className="cap-number">{number}</span><h3>{title}</h3><p>{body}</p><Arrow /></article></ElectricBorder>)}</div><ElectricBorder className="tool-panel" color="#b68aff"><div className="tool-strip"><span>TOOLS I USE</span><p>ChatGPT X ComfyUI X Midjourney X AE</p></div></ElectricBorder></section>
